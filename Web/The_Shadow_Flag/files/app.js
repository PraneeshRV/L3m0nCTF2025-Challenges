...
app.get('/checkout', (req, res) => {
    ...
    if (!carts[req.session.id]) {
        carts[req.session.id] = { item: null, discount: 0, coupons_applied: 0 };
    }
...
app.post('/cart/add', (req, res) => {
    ...
    carts[req.session.id] = { item: itemKey, discount: 0, coupons_applied: 0 };
...
// --- VULNERABILITY 2: RACE CONDITION (COUPON STACKING) ---
...
app.post('/checkout/pay', (req, res) => {
    ...
    if (user.balance >= finalPrice) {
        ...
        if (boughtItem === 'flag') {
            const flagValue = process.env.FLAG || 'l3mon{c0up0n_st4ck1ng_1s_th3_n3w_4rb1tr4g3}';
